...
internal class Spelbord : Panel {
  ...
  public bool FindEnclosure(int i, int j, int di, int dj,
  ...
  public void MuisClick(object sender, MouseEventArgs e) {
    ...
    if (CheckMove(x, y)) {
      ...
      MessageBox.Show(
          "I'm sorry, but that move is not legal! Perhaps try a hint?",
  ...
  public void DrawBoard(object? sender, PaintEventArgs e) {
    ...
    for (int i = 0; i < n; i++) {
      for (int j = 0; j < n; j++) {
        if (this.ShowHelp) {
          if (CheckMove(i, j)) {
            e.Graphics.DrawEllipse(new Pen(Color.Red), i * x + 5, j * y + 5,
                                   x - 10, y - 10);
          }
        }
        if (boardArray[i, j] == 1) {
  ...
  public int getPlayingPlayer() { return _playingPlayer; }
