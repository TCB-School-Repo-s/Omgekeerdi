...
internal class Spelbord : Panel {
   ...
   * flipped
   *
   * TODO: If one direction provides and illegal move, it doesn't continue to
   * check if the other directions have legal moves.
   */

  public bool FindEnclosure(int i, int j, int di, int dj, bool play = false,
                            bool first = true) {
  ...
  public void Play(int x, int y) {
    for (int i = -1; i <= 1; i++) {
      for (int j = -1; j <= 1; j++) {
        if (!(i == 0 && j == 0))
          FindEnclosure(x + i, y + j, i, j, true);
      }
    }
  }
  ...
  public void MuisClick(object sender, MouseEventArgs e) {
    ...
    Debug.WriteLine(
        $"board.Width: {board.Width} en board.Height: {board.Height}");
    ...
    if (CheckMove(x, y)) {
      ...
      MessageBox.Show("De move die wil is niet legaal", "Error!",
