...
internal class Spelbord : Panel {
  ...
  [DefaultValue(6), Category("1 Game Settings"),
   ...
   * and then returns an array with the first value containing if the move is
   * valid, and the second values represents the direction the stones should be
   * flipped
   *
   * TODO: The code doesn't work when using the 6x6 board, why? No clue, it
   * works on the all the sizes except 6x6
   */
  public Object[] LegalMove(int playerPlayer, int opponent, int x, int y) {
    ...
    if (boardArray[x, y] != 0) {
      ...
      foreach (Directions direction in Enum.GetValues(typeof(Directions))) {
        DirectionSpace _space =
            new DirectionSpace(x, y, 1, direction, boardArray, n);
        if (_space.ReturnSpace() == opponent) {
          Debug.WriteLine("-----------------------------------");
          Debug.WriteLine(
              $"Space has opponent next to it, Instance UUID: {Guid.NewGuid().ToString()}");
          Debug.WriteLine($"Direction checking: {direction.ToString()}");
          array[1] = direction;
          for (int j = 0; x + j < boardArray.GetLength(0) &&
                          y + j < boardArray.GetLength(1);
               j++) {
            Debug.WriteLine("i is within range");
            DirectionSpace space =
                new DirectionSpace(x, y, j, direction, boardArray, n);
  ...
  public void FlipStones(Directions direction, int playingPlayer, int opponent,
                         int x, int y) {
    for (int j = 0;
         x + j < boardArray.GetLength(0) && y + j < boardArray.GetLength(1);
         j++) {
      DirectionSpace space =
          new DirectionSpace(x, y, j, direction, boardArray, n);
      if (space.ReturnSpace() == playingPlayer) {
        ...
        boardArray[space.ReturnCoordinates().X, space.ReturnCoordinates().Y] =
            playingPlayer;
        this.Invalidate();
      }
    }
  ...
  public void DrawBoard(object? sender, PaintEventArgs e) {
    ...
    // draw black circle if bord[n,n] is 1 and draw white circle if bord[n,n] is
    // 2
    for (int i = 0; i < n; i++) {
      for (int j = 0; j < n; j++) {
        if (boardArray[i, j] == 1) {
          e.Graphics.FillEllipse(new SolidBrush(ColorOne), i * x + 5, j * y + 5,
                                 x - 10, y - 10);
        } else if (boardArray[i, j] == 2) {
          e.Graphics.FillEllipse(new SolidBrush(ColorTwo), i * x + 5, j * y + 5,
                                 x - 10, y - 10);
        }
      }
    }
...
internal class DirectionSpace {
  ...
  public DirectionSpace(int x, int y, int amount, Spelbord.Directions direction,
                        int[,] array, int n) {
    this.x = x;
    this.y = y;
    this.amount = amount;
